...
| ④ | 受信回数 | receiveCount | Label | 初回通知から現在までの１日あたりの通知回数。 NotificationLog.ReceivedCount / (現在日時 - NotificationLog.created + １日)| 
...
| ⑦ | ルール追加 | addRule | Button | 該当行のNotificationLog.PackageName, NtotificationLog.ChannelIDでRuleにレコードを追加する。詳細は後述のルール追加を参照のこと。 |
