## CL-0020 UX改善

### 変更内容

- 通知ログリスト開始位置統一 ([#14](https://github.com/Teyandei/SmartNotifire-Rev2/issues/14))  
- 音声案内の停止機能追加 ([#15](https://github.com/Teyandei/SmartNotifire-Rev2/issues/15))  
- ルール無効の視覚化 ([#16](https://github.com/Teyandei/SmartNotifire-Rev2/issues/16)) 

## CL-0019 Material 1.14.0 更新と Edge-to-Edge 対応

### 変更内容

- Material Components を 1.14.0 に更新
- Android 15 以降の Edge-to-Edge 表示に対応
- ステータスバー色の明示指定を削除
- メイン画面の上部・下部・左右端がシステムバーに被らないよう余白調整を追加
- 下部の通知確認欄がキーボード表示時に隠れないよう修正
- 通知タイトル条件の BottomSheet が横画面やチップ多段時にスクロールできるよう修正

### 確認内容

- 縦画面・横画面で表示確認
- ジェスチャーナビゲーション・3ボタンナビゲーションで表示確認
- Main画面、BottomSheet、キーボード表示時の下部入力欄を確認

## CL-0018 通知タイトル条件の詳細設定表示を調整

### 変更内容

- 「何れかを含む(OR条件)」が1件のみの場合は、通常の通知タイトル条件として表示するように変更
- 既存データに詳細形式のOR条件1件が保存されている場合も、ルール行では条件文字列そのものを表示するように変更
- OR条件1件のみのルールは、通常の通知タイトル条件と同様にコピーできるように変更

### 内部変更

- `TitleSearchCondition` に、詳細設定表示が必要かどうかを判定する処理を追加
- OR条件1件のみの詳細条件は、保存時に通常の通知タイトル条件文字列へ正規化するように変更
- OR条件1件のみの表示・保存・詳細判定に関する単体テストを追加

## CL-0017 通知タイトル条件の詳細設定追加

### 変更内容

- 通知タイトル条件に詳細設定を追加
  - 「絞り込み」「含まない」「何れかを含む」の条件を組み合わせて指定できるように変更
  - 条件はチップ表示にし、追加後に個別削除できるように変更
- ルール行の通知タイトル条件欄に詳細設定ボタンを追加
- 詳細条件設定中のルールは、通知タイトル条件欄に「詳細設定中」と表示するように変更
- 詳細条件の重複時は確定できないようにし、メッセージを表示するように変更
- 詳細条件のルールはコピーできないように変更
- 詳細設定の条件追加ダイアログで、通知タイトル候補を選択できるように変更
- 条件追加ダイアログの候補リスト幅を狭め、「追加」ボタンを確認しやすく調整
- アプリ名順のルール並び替えで、チャンネル名も考慮するように変更

### 内部変更

- 通知タイトル詳細条件を扱う `TitleSearchCondition` を追加
- 既存の単純な通知タイトル条件は、従来どおり部分一致条件として扱うように維持
- 詳細条件は既存の `RuleEntity.srhTitle` に保存できる形式へエンコードするように実装
- 通知受信時のルール判定を `TitleSearchCondition` 経由に変更
- 詳細条件設定用の BottomSheet と追加ダイアログを追加
- 詳細条件関連の文言を日英リソースに追加

## CL-0016 初回ユーザー向け案内の追加

### 変更内容

- 初めて使うユーザーが次の操作に迷わないよう、Getting Started ダイアログを追加
- 通知アクセス権限を許可した後、または既に許可済みの状態で起動したときに案内を表示
- 案内ダイアログから GitHub Pages の Getting Started ページを開けるように変更
- 「今後表示しない」により、案内の再表示を抑止できるように変更
- ヘルプダイアログの古い説明を見直し、GitHub Pages の使い方ページへ誘導する内容に整理
- 通知タイトル条件の入力候補が、初回タップ時にも表示されるように修正

### 内部変更

- `show_getting_started` を AppPrefs に追加
- ダイアログ内リンク処理とチェックボックス付き本文 View の生成処理を共通化

## CL-0015 通知タイトル条件の表記整理

### 変更内容

- 音声案内ルール内の「検索タイトル」表記を、日本語表示では「通知タイトル条件」に変更
- 同一アプリ内の重複警告メッセージも「通知タイトル条件」に表記を統一
- ルール編集ページの日本語ドキュメントで、「検索タイトル」表記を「通知タイトル条件」に修正
- フォーカスアウト時にDB更新される現在の仕様に合わせて、ルール編集ページの古い注意書きを削除
- タイポミスを修正。「新帰順」-> 「新着順」

## CL-0014 音声案内ヒントの追加

### 変更内容

- 音声案内が聞こえない場合の確認ダイアログを追加
- 有効ルール作成後の案内表示と、再表示抑止設定を追加

## CL-0013 バグ修正

### 変更内容  

- 初回起動時のルール初期レコードにチャンネル名の設定忘れを修正
- 日本語ページ作成

## CL-0012 設計書の現状反映

### 変更内容

- CL-0011時点の実装コードを設計書に反映
- Gradle v9.4.1アップデートによる自動更新

## CL-0011 通知ログの自動整理と入力補助の強化

### 変更内容

- **通知ログの自動クリーンアップ機能の追加**
  - 90日が経過した古い通知ログを毎日自動でバックグラウンド削除する機能を追加。
- **通知タイトルの履歴保存（キャッシュ）機能の導入**
  - 受信した通知のタイトルを自動的に記録し、ルール作成時の入力候補として利用可能に改善。
- **通知ログリストの視覚的改善**
  - 既にルールが設定されている通知や、サイレント通知（重要度が低いもの）をリスト上で色分けして表示し、現在の状態を判別しやすく改善。
- **ダークモード表示の最適化**
  - 夜間モードにおけるカラーリソースを調整し、視認性を向上。

### その他

- **データベースの構成変更**
  - タイトルキャッシュ保存用のテーブル追加に伴うマイグレーション処理の実施。

## CL-0010

### 更新内容

- 通知地ログリストの多種類ソート対応
- デバッグ・リリースバージョンの同一デバイス配置対応
- DBのexportSchema = trueでスキーマファイルを正しく出力する設定を追加
- Gradleバージョンアップ対応(システム自動)

## CL-0009 ルール追加行の表示

### 変更内容

- 通知ログのダブルタップで追加したルールを見える位置にスクロールする。
- ルールのコピーで追加したルールを見える位置にスクロールする。
- Gradle v9.0.1 アップデートによる自動更新。

## CL-0008 通知モニタによる改修 2026-02-11

### 変更内容

- 通知テーブル(Notifications)追加と記録
- 通知がonGoing(連続的な通知)であるものを無視
- 重要度(Importance)によるTTS使用判断を追加(IMPORTANCE_DEFAULT以上の通知のみTTS)
- 通知ログにImportance、channelName, created、lastReceivedを追加
- 通知ログリスト表示にchannelName, 受信件数(件/日)、サイレント(IMPORTANCE_DEFAULT未満)を追加
- サイレント通知のルール追加に警告ダイアログで注意を促す
- その他改修

## CL-0007 通知ログ改修　他 2026-02-01

### 変更内容

- 通知ログ改修(CL-0006)の実装

- gradle 9.0.0 適用による変更

## CL-0006 アプリ目的の明確化　2026-01-29

### 変更内容

今回はアプリ目的の明確化によるEntity変更による規模の修正の前段階として、設計書及びREADME.md改修のみとする。

### 背景

通知ログ観測結果と、MIUI系OSの通知リスナー権限画面の「危険」表示からでたことによる方針変更として、

**通知ログからタイトルを記録しない**  

とした。それは以下の理由による。  

   1. CL-0005改修によりタイトル無しでTTSできる。
   2. タイトルはプライバシーに関する内容がある。
   3. 副産物として、保存コストの削減が期待できる。

## CL-0005 通知の取り扱いに関する改修 2026-01-24

### 背景

- PackageNameからPackageManagerによるアプリ名取得(label)はどうも時間経過とともに取得できない現象がある。
- それによって、CL-0003の改修を行ったが、そのままでは使えないルールが多発する。

### 改修のポイント

- 通知リスナーサービスでPackageNameで取得できたアプリ名だけを本アプリの処理対象とする。
- アプリ名取得は、通知リスナーで取得したアプリ名を通知ログの項目としてアプリ名(appLabel)を新規追加。
- 今まで、PackageManagerで各所アプリ名変換で使用してきた部分をDBのアプリ名から取得してルールの延命を図る。
- 通知リスナーサービスのアクセス権限を有効にしたくないユーザーに対しては、本アプリの終了を選択できるようにした。
- 設計書に記載の検索タイトル未設定は、そのアプリの通知全体を検索ヒット対象にした、これは設計書の当初の仕様である。
- 音声メッセージ(VoiceMag)がemptyの場合は、<アプリ名>の通知が届きましたという音声読み上げを行うように修正した、
- 以上の改修により、設計書文言、README.mdの文言を修正した。

### その他の改修

- 通知ログ、ルールに採用していた一意制約インデックスをタイトル、通知メッセージの初期値をnullからemptyにした。
- 一意政策インデックスのタイトルについて、重複を避ける付与文字を(n)から-#nnにした。
- ルールのコピーと通理ログによる２系統のルール追加処理を一本化した、

## CL-0004 マナーモード対応、バグ修正 2026-01-23

### マナーモード対応

- マナーモードの場合はTTSを出力しない。

### バグ修正

- 現象  
  ルール内許可スイッチの状態がDB反映されない。したがって通知しても音声案内が出力されない。

- 原因  
  RulesAdapterで、CL-0003対応時に通常バインドを許可スイッチに入れていなかった。

- 対応  
  ViewHolderのbind関数内にバインドするコードを追加

## CL-0003 無効通知の整理 2026-01-22

### 現象

- 追加ボタンクリックによる通知ログ表示で、アプリ名に変換できず、パッケージ名＋デフォルトアイコンのレコードがある。

### 原因

- PackageManager.NameNotFoundException例外が発生するも、パッケージ名とデフォルトアイコンを表示するコードになっていた。
- 以下の点が直接原因と考えられる。
  1. プリインストール系、一時的なメーカーアプリ、disabled / hidden 状態のアプリは、getApplicationInfo(packageName, 0) が 例外を投げる or null相当 になることがある
  2. 表示時点でアンインストール/無効化されて、後から変換が失敗
  3. 別ユーザー（Work profile / サブユーザー）由来の通知
  4. パッケージは見えるがリソース参照が失敗（まれに Resources.NotFoundException 系）

### 対処
- PackageManager.NameNotFoundException例外が発生した場合は、通知ログの該当行削除依頼をNotificationLogAdapterに追加
- NotificationLogAdapter->MainFragment->MainViewModel->NotificationLogRepository->NotificationDaoで該当行を削除
- Ruleに既にある無効なレコードに対してenabledをオフにする処理を追加。アプリ名に無効であることを表示。（削除判断はユーザーに任せる）
- その他クリティカルバグの修正

## CL-0002 プライバシーポリシー変更 - 2026-01-20

### 変更内容
- Google Play Storeのデフォルト言語が`ja-JP`のため、プライバシーポリシーを日本語版リンクに変更。よって下記の修正をした。
  1. 日本語プライバシーポリシーに英語版へのリンクを文末に追加。
  2. 英語版プライバシーポリシーに日本語版へのリンクを文末に追加
- フィーチャーグラフィックを追加


## CL-0001 Internal Test Start - 2026-01-19

### 変更内容
- ソースコード内のクラス・メソッドに KDoc コメントを追加
- 追加ボタンで表示する通知ログリストに✕(閉じる)ボタンを追加。
- 追加ボタンでダブルタップする新しいルールの重複タイトル自動生成をカウントから、50回ループによるナンバリングに変更。
- 同上において、ナンバリングの表示形式を`(n)`から`-#nn`に変更。
- MainViewModel内の日本語出力部分を国際化対応。
  1. MainViewModelにsealed interface AddRuleFromLogEventを追加
  2. MainViewModelでは文言を生成せず、AddRuleFromLogEvent として結果のみを通知（StateFlowを公開）
  3. MainFragmentで購読して、string.xmlによる国際対応メッセージに変換してSnackBarに出力
